...
- **`react-guidepopover-ignores-instance-i18n`** — ~~<GuidePopover> reads the module-global defaultI18n, so gf.i18n.use()/register() has no effect on its strings~~ **RESOLVED.** It reads `gf.i18n` at [GuidePopover.tsx:206](../../packages/react/src/components/GuidePopover.tsx#L206). The non-reactivity half is documented intended behaviour, consistent with core — a locale change appears on the next render, not on the step already on screen. The warning outlived the fix in `CLAUDE.md` §5.5 and `apps/docs/guide/i18n.md`; both corrected 2026-08-07.  
